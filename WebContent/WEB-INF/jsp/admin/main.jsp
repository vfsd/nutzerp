  ...
  <body>
      <section>
      		<div id="cc" class="easyui-layout" style="width:100%;height:800px;">
			    ...
			    <div data-options="region:'west',title:'系统管理',split:true,animate:true" style="width:180px;">
                    <div title=""  id="left_menu">
                    	<ul id="tt" class="easyui-tree">
                            <li><a data-url="<%=basePath%>admin/toUser.action" >用户管理</a></li>
                            <li><a data-url="<%=basePath%>admin/toAccount.action" >财务人员管理</a></li>
						</ul>
                    </div>
			    </div>
			    <div data-options="region:'center'" style="padding:5px;background:#eee;">
			    	<div id="main_menu" class="easyui-tabs" data-options="region:'center'" style="height:460px;overflow:hidden;">
			    		<div class="user_msg" data-options="iconCls:'icon-help',title:'首页'" >
			    			...
			    			<h4>姓名：<%=request.getSession().getAttribute("loginAdminName") %></h4>
...
<script>
	...
	function addTab1(tt,t_url,t_icon){
				//alert("ok");
				var t_title=tt;
				var tt_url=t_url;
				//alert(t_url);
				...
				var u_n= new Array();
				u_n=strs[1].split("=");
				var tt_url=strs[0]+"?user_name="+u_n[1];
				//alert(tt_url);
				var t_flag=$("#main_menu").tabs('exists',t_title);
				$("#main_menu").tabs('add',{title :t_title,closable : true,fit:true,content:'<iframe src='+tt_url+' style="width:100%;height:100%;border:none"></iframe>'});
	}
	
	function addTabRPActive(tt,t_url,t_icon){
			//alert("ok");
			var t_title=tt;
			var tt_url=t_url;
			//alert("t_title"+t_title);
			//alert("tt_url="+tt_url);
			...
			//alert("username="+u_n[1]);
			
			//var tt_url=strs[0]+"?userNN="+u_n[1];
			var tt_url=strs[0]+"?SAID="+u_n[1];
			//alert("----tt_url-----=-----"+tt_url);
