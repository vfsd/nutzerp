...
<script>
     function initTable(){
        $("#dg").datagrid({
           url:'<%=basePath%>news/query.action',
           idField:'id',
           fitColumns:true,
           pagination:true,
           toolbar:'#tb',
           ...
           columns:[[  
                ...
                {field:'state',title:'状态',width:200,
                	formatter:function(value,row,index){
                		...
                		if(value==0){
                			return "正常显示";//0:未读   1:已接受  2:驳回
                		}
                		if(value==1){
                			return "禁止显示";//0:未读   1:已接受  2:驳回
                		}
                	}	
                },
                {field:'content',title:'公告内容',width:600}
           ]]
       });
   }
...
<div id="inputDialog">
	<form method="post">
		<div style="text-align:center;">
			<textarea rows="3" cols="80" name="content" placeholder="请输入公告内容" required="required" style="margin:0 auto;"></textarea>
		</div>
	</form>
</div>
...
<div id="editDialog">
	<form method="post">
		...
		<div style="text-align:center;">
			<textarea rows="3" cols="80" name="content" placeholder="请编辑公告内容" required="required" style="margin:0 auto;"></textarea>
		</div>
	</form>
</div>
     ...
     function delClass(){
        ...
        $.messager.confirm("信息提示","确实要删除该条预约请求么？",function(flag){
            if(flag){
                ...
                $.post("<%=basePath%>news/delete.action",param,function(result){
